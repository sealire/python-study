...
class GirlAtlasXrwang:

    def get_website_info(self):
        return {
            "title": "girl-atlas-Xrwang",
            "url_template": "https://www.xrwang.vip/page/{page}/",
        }
    ...
    def check_page_exist(self, thread_id, page, page_url):
        ...
        if not html_text:
            return False
        container = html_text.find('div', id='index-tab-main')
        if not container:
            return False
        model_cards = container.find_all("div", class_='item-body')
        if model_cards:
            return True
        else:
            return False
    ...
    def get_models(self, thread_id, page, page_url, model_names):
        ...
        for index, model_card in enumerate(model_cards):
            ...
            model_card_h4 = model_card.find("h2").find("a")
